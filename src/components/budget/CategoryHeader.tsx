...
export function CategoryHeader({ category, subtotal, sectionCount, itemCount }: CategoryHeaderProps) {
  return (
    <div className="mt-8 first:mt-0 mb-3">
      <div className="flex items-center gap-3">
        <div className={cn("w-1 self-stretch rounded-full", category.bgClass)} />
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between gap-2">
            <span className={cn("text-base font-display font-bold tracking-tight", category.colorClass)}>
              {category.label}
            </span>
            <span className="font-display font-bold text-base text-foreground tabular-nums flex-shrink-0">
              {formatBRL(subtotal)}
            </span>
          </div>
