    ...
    <>
      ...
      <div className="rounded-xl overflow-hidden border border-border bg-card shadow-sm hover:shadow-md transition-shadow">
        ...
        {section.cover_image_url ? (
          ...
          >
            ...
            <div className="absolute bottom-0 left-0 right-0 p-5">
              <h2 className="font-display text-xl sm:text-2xl font-bold text-card">{section.title}</h2>
              {section.subtitle && (
                <p className="text-card/80 text-sm mt-1 font-body">{section.subtitle}</p>
              )}
            </div>
          </div>
        ) : (
          <div className="relative h-32 bg-gradient-to-br from-muted to-accent flex items-center justify-center">
            <ImageIcon className="h-8 w-8 text-muted-foreground/40" />
            <div className="absolute bottom-0 left-0 right-0 p-5">
              <h2 className="font-display text-xl sm:text-2xl font-bold text-foreground">{section.title}</h2>
              {section.subtitle && (
                <p className="text-muted-foreground text-sm mt-1 font-body">{section.subtitle}</p>
              )}
            </div>
          </div>
        )}
        ...
        <div className="px-5 py-3 border-b border-border bg-accent/30 flex items-center justify-between">
          ...
          <span className="font-display font-bold text-lg text-primary">{formatBRL(subtotal)}</span>
        </div>
        ...
        <div className="p-5">
          {items.length > 0 && (
            <>
              <h3 className="text-sm font-semibold text-muted-foreground uppercase tracking-wider mb-3 font-body">
                Itens Inclusos
              </h3>
              ...
              {items.length > 3 && (
                <button
                  ...
                  className="mt-3 flex items-center gap-1.5 text-sm text-primary hover:text-primary/80 font-medium font-body transition-colors"
                >
                  {expanded ? (
                    <>
                ...
                <div>
                  ...
                  <ul className="space-y-1.5">
                    {excluded.map((b: string, i: number) => (
                      <li key={i} className="text-sm text-foreground/60 font-body flex items-start gap-2">
          ...
          {/* Section notes */}
          {section.notes && (
            <p className="mt-4 pt-3 border-t border-border text-xs text-muted-foreground font-body italic">
              {section.notes}
            </p>
          )}
        </div>
