...
- Sync trio `sync-supplier-outbound` (envision→portal, wrapper `{ fornecedor, source_id }`), `sync-suppliers-inbound` (plural, **endpoint que o Portal BWild realmente chama**, payload achatado `{ name, _source_system, _source_id, ... }` → responde `{ success, target_id }`) e `sync-supplier-inbound` (singular, mantido só por compatibilidade), mais `sync-project-outbound` (dispara em `internal_status → contrato_fechado`) ↔ Portal BWild. Auth é `x-integration-key`, **não** JWT — a chave do Envision (`INTEGRATION_INBOUND_KEY`) precisa bater com a do Portal (`INTEGRATION_API_KEY`). Idempotência via `integration_sync_log` UNIQUE em `(source_system, entity_type, source_id)`. Ver `docs/envision-integration-reference.md` e `docs/envision-budget-payload-schema.md`.
