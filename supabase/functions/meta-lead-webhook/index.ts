 ...
 * A ativação completa será feita futuramente quando o app do Meta for configurado.
 *
 * Fluxo previsto:
 *  1. Meta envia GET com `hub.challenge` para verificação inicial → respondemos com o challenge.
 ...
 *  4. Para cada lead:
 *     - Persistimos o payload bruto em `lead_sources` (auditoria/reprocessamento).
 *     - Buscamos os dados completos do lead na Graph API (quando META_PAGE_ACCESS_TOKEN existir).
 *     - Fazemos upsert em `clients` (deduplicando por external_source + external_lead_id).
 *     - O trigger `create_mql_budget_for_new_client` cria automaticamente o orçamento MQL.
 *
 * Segredos necessários para ativação (NÃO são obrigatórios agora — a função
 * responde com 503 caso não estejam configurados):
 *  - META_APP_SECRET           → assinatura HMAC dos webhooks
 *  - META_VERIFY_TOKEN         → string usada na verificação inicial do webhook
...
Deno.serve(async (req) => {
  ...
  // Se ainda não temos secrets configurados, ainda assim registramos o payload
  // bruto para auditoria (útil para debug durante o setup) e respondemos 200.
  ...
  for (const entry of payload.entry ?? []) {
    for (const change of entry.changes ?? []) {
      ...
      try {
        // 1. Persiste payload bruto SEMPRE (auditoria)
        ...
        if (existingLead?.processing_status === "processed") {
          results.push({ leadgen_id: leadgenId, status: "duplicate" });
          continue;
        }

        // 2. Enriquece via Graph API se token disponível
        ...
        const leadSourceRow = {
          ...
          processing_status: pageAccessToken ? "pending" : "pending",
        };

        const { data: leadSource, error: leadSourceErr } = await supabase
          .from("lead_sources")
          .upsert(leadSourceRow, { onConflict: "source,external_id" })
          .select("id")
          .single();
        if (leadSourceErr) throw leadSourceErr;

        // 3. Sem token de página, paramos aqui (registro feito, processamento posterior)
        ...
        const clientPayload = {
          ...
          external_lead_payload: fields,
          campaign_id: leadFull.campaign_id ?? null,
          campaign_name: leadFull.campaign_name ?? null,
          adset_id: leadFull.adset_id ?? null,
          adset_name: leadFull.adset_name ?? null,
          ad_id: leadFull.ad_id ?? null,
          ad_name: leadFull.ad_name ?? null,
          form_id: leadFull.form_id ?? null,
          utm_source: "meta",
        ...
        const { data: client, error: clientErr } = await supabase
          .from("clients")
          .upsert(clientPayload, { onConflict: "external_source,external_lead_id" })
          .select("id")
          .single();
        if (clientErr) throw clientErr;

        // 5. Marca como processado e linka client_id
        await supabase
          .from("lead_sources")
          .update({
            client_id: client.id,
        ...
        results.push({ leadgen_id: leadgenId, status: "processed" });
      } catch (err) {
        const message = err instanceof Error ? err.message : "Unknown error";
        console.error(`Failed to process leadgen ${leadgenId}:`, message);
        await supabase
          .from("lead_sources")
          .update({
          ...
          .eq("source", "meta_ads")
          .eq("external_id", leadgenId);
        results.push({ leadgen_id: leadgenId, status: "failed", error: message });
      }
    }
  }
